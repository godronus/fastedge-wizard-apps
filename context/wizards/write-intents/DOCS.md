...
cd wizards/write-intents && pnpm run dev
```

`dev` builds once and starts the SDK's `bin/dev.mjs` mock host server. The mock
host serves the wizard inside a real iframe bridge at `http://localhost:9999/mock-host`,
loading fixtures from `fixtures/` at startup and using them to respond to intents
instead of calling the real API. For file watching during development, run
`pnpm run dev:watch` in a second terminal (builds only — no host restart needed).
